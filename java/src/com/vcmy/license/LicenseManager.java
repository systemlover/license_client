...
public class LicenseManager {
	...
	public boolean verify(byte[] licenseContent, boolean checkPubkey) throws Exception {
		...
		X509Certificate ca_cert = loadCACertificate();
		X509Certificate client_cert = loadPemX509Certificate(licenseContent);
		if (ca_cert.getSubjectX500Principal().equals(client_cert.getIssuerX500Principal())) {
			try {
				client_cert.verify(ca_cert.getPublicKey());
				valid = true;
			} catch (Exception e) {
				valid = false;
			}
		...
		if (valid && checkPubkey) {
			KeyPair keyPair = this.getKeyPair();
			valid = keyPair.getPublic().equals(client_cert.getPublicKey());
		}
		
		// check if client_cert not expired
		if (valid) {
			Date now = new Date();
			valid = client_cert.getNotAfter().compareTo(now) > 0;
		}
